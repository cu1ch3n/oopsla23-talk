...
If we define two identity functions, idFun is from type B to type A to A, but the bound of B depends on A, another one is from Int to Int. In Scala 2, we cannot apply idFun to idInt1, because Scala 2 wrongly instantiates A with bottom (which is Nothing in Scala) instead of Int.\nl
...
\begin{frame}[fragile]{Contributions}
    \begin{itemize}
		\item A declarative bidirectional type system
			...
			\end{itemize}
		\item An algorithmic formulation of the type system
			\begin{itemize}
				\item sound, complete and decidable
				\item worklist formulation\footnote{Jinxu Zhao, Bruno C. d. S. Oliveira, and Tom Schrijvers. A Mechanical Formalization of Higher-Ranked Polymorphic Type Inference. ICFP 2019.}
			\end{itemize}
		\item A variant of \name with monotype subtyping
			\begin{itemize}
				\item at the cost of completeness 
			\end{itemize}
		\item Completeness with respect to kernel $F_{\le}$
        \item Mechanical formalization and implementation
              \begin{itemize}
              	\item All are verified in Abella (LOC: 24,919)
